...
#ifdef __HIDE_COSTUME_SYSTEM__
	// Body-Kostum ausblenden?
	if (bPartPos == PART_MAIN
		&& GetWear(WEAR_COSTUME_BODY)
		&& IsBodyCostumeHidden())
	{
		if (LPITEM pArmor = GetWear(WEAR_BODY))
			return pArmor->GetVnum();
		else
			return 0;
	}
	// Haar-Kostum ausblenden?
	else if (bPartPos == PART_HAIR
			&& GetWear(WEAR_COSTUME_HAIR)
			&& IsHairCostumeHidden())
	{
		return 0;
	}
	// Accessoire-Kostum ausblenden?
	else if (bPartPos == PART_ACCE
			&& GetWear(WEAR_COSTUME_ACCE)
			&& IsAcceCostumeHidden())
	{
		return 0;
	}
	// Waffen-Kostum ausblenden?
	else if (bPartPos == PART_WEAPON
			&& GetWear(WEAR_COSTUME_WEAPON)
			&& IsWeaponCostumeHidden())
	{
