...
{
	bool b;
	if (!PyTuple_GetBoolean(poArgs, 0, &b))
		return Py_BuildException();

	CPythonApplication::Instance().SetReloadLocale(b);
	return Py_BuildNone();
}
...
#if defined(__BL_MULTI_LANGUAGE__)
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE__", true);
#else
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE__", false);
#endif

#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE_PREMIUM__", true);
#else
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE_PREMIUM__", false);
#endif

#if defined(__BL_MULTI_LANGUAGE_ULTIMATE__)
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE_ULTIMATE__", true);
#else
	PyModule_AddIntConstant(poModule, "__BL_MULTI_LANGUAGE_ULTIMATE__", false);
#endif

#ifdef __PREMIUM_PLAYER__
	PyModule_AddIntConstant(poModule, "__PREMIUM_PLAYER__", 1);
#else
	PyModule_AddIntConstant(poModule, "__PREMIUM_PLAYER__", 0);
#endif
...
//martysama0134's b3d235073c17370fd346b2e61d8c8267
